    ...
    [
        [
            InlineKeyboardButton(
                text="اشترك هنا", url="https://t.me/creativee4"
            ),                        
        ],        
    ]
)
...
async def check_is_joined(message):    
    try:
        ...
        status = await app.get_chat_member("creativee4", userid)
        return True
    except Exception:
        await message.reply_text("*انت لست مشترك في قناة البوت @creativee4 ** \n**انضم لتستطيع تشغيل الاغاني**",reply_markup=force_btn,parse_mode="markdown",disable_web_page_preview=False)
        return False
